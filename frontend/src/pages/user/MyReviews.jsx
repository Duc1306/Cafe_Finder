...
export default function MyReviews() {
  ...
  const handleEdit = async () => {
    try {
      ...
      setReviews(reviews.map(r => r.id === editing ? { ...r, ...editData } : r));
      setEditing(null);
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      ...
      {filtered.map(r => (
        <div key={r.id} className="bg-white rounded-xl shadow p-6 mb-6 flex flex-col">
      ...
      >
        <div className="mb-4">
          ...
          <Rate value={editData.rating} onChange={v => setEditData(d => ({ ...d, rating: v }))} />
        </div>
        <Input.TextArea
          rows={4}
